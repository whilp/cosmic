...
Type declarations for the `finger` module.
...
```teal
function FingerSyn(syn_packet_bytes: string): number
```
...
```teal
function GetSynFingerOs(synfinger: number): "LINUX"
```
...
```teal
function DescribeSyn(syn_packet_bytes: string): string
```
