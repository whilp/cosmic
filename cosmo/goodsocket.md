...
Type declarations for the `goodsocket` module.

## Types

### goodsocket Constants

Constants defined in the goodsocket module.

```teal
local record goodsocket Constants
  AF_INET: number
  AF_INET6: number
  AF_UNIX: number
  AF_UNSPEC: number
...
```teal
function socket(family: number, type: number, protocol: number, isserver: boolean, timeout?: number): number
```
